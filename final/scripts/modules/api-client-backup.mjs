...
// Simplified KISS implementation with URL-based requests
...
async function getAccessToken() {
    ...
    try {
        ...
        if (error.message.includes('fetch') || error.message.includes('Failed to fetch') || error.message.includes('CORS')) {
            console.info('🔒 [API CLIENT] FatSecret API blocked by CORS (normal in browser environment)');
        } else {
            console.error('❌ [API CLIENT] OAuth error:', error.message);
        }
        throw error;
    }
}
...
/**
 * Make authenticated request to FatSecret API using URL-based method
 */
async function makeApiRequest(endpoint, params = {}) {
    try {
        ...
        console.log('🌐 [API CLIENT] Making request to:', url.toString());
...
export async function getRecipeById(recipeId) {
    ...
    try {
        // Try real API first using URL-based method
        ...
        if (data.recipe) {
            ...
            console.log(`✅ [API CLIENT] Detailed recipe adapted successfully`);
        ...
        // Expected CORS error in browser environment
        if (error.message.includes('fetch') || error.message.includes('CORS') || error.message.includes('Failed to fetch')) {
            console.info(`� [API CLIENT] CORS blocked detailed fetch for ID ${recipeId} (this is expected in browser)`);
            console.info('📋 [API CLIENT] Will use existing search/daily data instead');
        } else {
            console.warn(`⚠️ [API CLIENT] API error for detailed fetch:`, error.message);
        }
        return null;
    }
}
...
export async function getDailyRecipes() {
    ...
    try {
        console.log('🍽️ Fetching daily recipes...');
        ...
        const source = recipes.length > 0 && recipes[0].id.startsWith('api_') ? 'API' : 'Fallback';
        console.log(`✅ Loaded ${dailyRecipes.length} recipes from ${source}`);
...
export async function searchRecipes(searchTerm) {
    ...
    try {
        ...
        const source = results.length > 0 && results[0].id.startsWith('api_') ? 'API' : 'Fallback';
        console.log(`✅ Found ${preservedResults.length} recipes from ${source}`);
